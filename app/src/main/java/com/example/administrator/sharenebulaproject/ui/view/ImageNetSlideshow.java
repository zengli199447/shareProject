...
import com.example.administrator.sharenebulaproject.model.bean.ImageTitleBean;
import com.example.administrator.sharenebulaproject.model.bean.ImageTitleNetBean;
 ...
 * 自定义轮播图(网络)
 */
public class ImageNetSlideshow extends FrameLayout {
    ...
    private List<ImageTitleNetBean> imageTitleBeanList;
    private int dotSize = 12;
    private int dotSpace = 12;
    private int delay = 3000;
    private MyViewPager vpImageTitle;
    private boolean LastStatus;

    public ImageNetSlideshow(Context context) {
        this(context, null);
    }

    public ImageNetSlideshow(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ImageNetSlideshow(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        this.context = context;
        initImageNetSlideshow();
    }

    public void initImageNetSlideshow(){
        // 初始化View
        initView();
        // 初始化Animator
        initAnimator();
    ...
    private void initView() {
        contentView = LayoutInflater.from(context).inflate(R.layout.image_slide_show, this, true);
        vpImageTitle = (MyViewPager) findViewById(R.id.vp_image_title);
        llDot = (LinearLayout) findViewById(R.id.ll_dot);
    }

    // 设置小圆点的间距
    public void setOverStatus(boolean LastStatus) {
    ...
    public void addImageUrl(String imageUrl) {
        ImageTitleNetBean imageTitleBean = new ImageTitleNetBean();
    ...
    // 添加图片和标题的JavaBean
    public void addImageTitleBean(ImageTitleNetBean imageTitleBean) {
        imageTitleBeanList.add(imageTitleBean);
    }

    // 设置图片和标题的JavaBean数据列表
    public void setImageTitleBeanList(List<ImageTitleNetBean> imageTitleBeanList) {
     ...
     */
    private void setViewPager(List<ImageTitleNetBean> imageTitleList) {
        // 设置View列表
        setViewList(imageTitleList);
        ...
        vpImageTitle.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(final int position) {
                ...
                if (LastStatus && position == imageTitleBeanList.size())
                    MyApplication.executorService.submit(new Runnable() {
                        @Override
                        public void run() {
                            try {
                                Thread.sleep(1999);
                                RxBus.getDefault().post(new CommonEvent(EventCode.OVERINSERT_NEW_TASK, position));
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        }
     ...
     */
    private void setViewList(List<ImageTitleNetBean> imageTitleBeanList) {
        //图片类型设置 type : 0正常 ，1圆角 ，2 圆形
        viewList = new ArrayList<>();
        for (int i = 0; i < count + 2; i++) {
            View view = LayoutInflater.from(context).inflate(R.layout.image_slide_show_item, null);
            ImageView ivImage = (ImageView) view.findViewById(R.id.iv_image);
            TextView tvTitle = (TextView) view.findViewById(R.id.tv_title);
            if (i == 0) {// 将最前面一页设置成本来最后的那页
//                Glide.with(context).load(imageTitleBeanList.get(count - 1).getImageUrl()).asBitmap().placeholder(R.drawable.banner_off).into(ivImage);
                Glide.with(context).load(imageTitleBeanList.get(count - 1).getImageUrl()).centerCrop().error(R.drawable.banner_off).into(ivImage);
                tvTitle.setText(imageTitleBeanList.get(count - 1).getTitle());
            } else if (i == count + 1) {// 将最后面一页设置成本来最前的那页
//                Glide.with(context).load(imageTitleBeanList.get(0).getImageUrl()).asBitmap().placeholder(R.drawable.banner_off).into(ivImage);
                Glide.with(context).load(imageTitleBeanList.get(0).getImageUrl()).centerCrop().error(R.drawable.banner_off).into(ivImage);
                tvTitle.setText(imageTitleBeanList.get(0).getTitle());
            } else {
//                Glide.with(context).load(imageTitleBeanList.get(i - 1).getImageUrl()).asBitmap().placeholder(R.drawable.banner_off).into(ivImage);
                Glide.with(context).load(imageTitleBeanList.get(i - 1).getImageUrl()).centerCrop().error(R.drawable.banner_off).into(ivImage);
