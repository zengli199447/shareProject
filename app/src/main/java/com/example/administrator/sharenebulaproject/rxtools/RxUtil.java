package com.example.administrator.sharenebulaproject.rxtools;

import com.example.administrator.sharenebulaproject.model.http.exception.ApiException;
import com.example.administrator.sharenebulaproject.model.http.response.MyHttpResponse;
            ...
            public Flowable<T> apply(Flowable<MyHttpResponse<T>> httpResponseFlowable) {
                return httpResponseFlowable.flatMap(new Function<MyHttpResponse<T>, Flowable<T>>() {
                    ...
                    public Flowable<T> apply(MyHttpResponse<T> tMyHttpResponse) {
                        ...
                        if(tMyHttpResponse.getCode() == 1) {
                            return createData(tMyHttpResponse.getEntity());
                        } else {
